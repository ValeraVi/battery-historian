...
func parseAppSensor(record []string, app *bspb.BatteryStats_App) (string, []error) {
	...
	for _, s1 := range app.Sensor {
		if s1.GetNumber() == s.GetNumber() {
			s1.TotalTimeMsec = proto.Float32(s1.GetTotalTimeMsec() + s.GetTotalTimeMsec())
			s1.Count = proto.Float32(s1.GetCount() + s.GetCount())
			return warn, nil
		}
	}
...
//
// format: 9,10007,l,jb,com.google.android.gms/.gcm.nts.TaskExecutionService,14000,3
...
func parseAppWakelock(c checkinutil.Counter, reportVersion int32, record []string, app *bspb.BatteryStats_App) (string, error) {
	...
	var fMax, fCur, pMax, pCur, wMax, wCur int64
	...
	for _, w1 := range app.Wakelock {
		if w1.GetName() == name {
			...
			w1.FullCurrentDurationMsec = proto.Int64(maxInt64(w1.GetFullCurrentDurationMsec(), fCur))
			w1.FullMaxDurationMsec = proto.Int64(maxInt64(w1.GetFullMaxDurationMsec(), fMax))
			w1.PartialTimeMsec = proto.Float32(w1.GetPartialTimeMsec() + pt)
			w1.PartialCount = proto.Float32(w1.GetPartialCount() + pc)
			// Current and max should only track the longest value for the wakelock.
			w1.PartialCurrentDurationMsec = proto.Int64(maxInt64(w1.GetPartialCurrentDurationMsec(), pCur))
			w1.PartialMaxDurationMsec = proto.Int64(maxInt64(w1.GetPartialMaxDurationMsec(), pMax))
			w1.WindowTimeMsec = proto.Float32(w1.GetWindowTimeMsec() + wt)
			w1.WindowCount = proto.Float32(w1.GetWindowCount() + wc)
			// Current and max should only track the longest value for the wakelock.
			w1.WindowCurrentDurationMsec = proto.Int64(maxInt64(w1.GetWindowCurrentDurationMsec(), wCur))
			w1.WindowMaxDurationMsec = proto.Int64(maxInt64(w1.GetWindowMaxDurationMsec(), wMax))
			return warn, nil
		}
	}
...
func parseSystemKernelWakelock(record []string, system *bspb.BatteryStats_System) (string, []error) {
	...
	for _, kw := range system.KernelWakelock {
		if kw.GetName() == w.GetName() {
			...
			// Current and max should only track the longest value for the wakelock.
			kw.CurrentDurationMsec = proto.Int64(maxInt64(kw.GetCurrentDurationMsec(), w.GetCurrentDurationMsec()))
			kw.MaxDurationMsec = proto.Int64(maxInt64(kw.GetMaxDurationMsec(), w.GetMaxDurationMsec()))
			return warn, nil
		}
	}
